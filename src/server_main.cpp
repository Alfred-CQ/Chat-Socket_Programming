...
#define PORT "45000" // port we're listening on
...
int main(void)
{
    ...
    printf("Socket Server Listen from #%s \n", PORT);
        ...
        for (i = 0; i <= fdmax; i++)
        {
            if (FD_ISSET(i, &read_fds))
            {
                ...
                else
                {
                    ...
                    n = recv(i, &action, 1, 0);
                    if (action == 'N')
                        myServer.register_client(i);
                    else
                    {
                        int idx;
                        for (idx = 0; idx < myServer.s_clients.size(); ++idx)
                        {
                            if (i == myServer.s_clients[idx]->scli_socketFD)
                                break;
                        }

                        manager(myServer.s_clients[idx], action);
                    }
                }
            }
        }
